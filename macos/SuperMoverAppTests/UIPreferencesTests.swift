...
final class UIPreferencesTests: XCTestCase {
    ...
    func testAppChromeLocalizationLoadsPrepareChromeResources() {
        ...
        XCTAssertEqual(english.text(.setupTargetRootFieldTitle), "Destination on Target Mac")
        XCTAssertEqual(simplifiedChinese.text(.setupTargetRootFieldTitle), "目标 Mac 保存位置")
        XCTAssertEqual(english.text(.setupTargetRootPlaceholder), "Enter the folder path on the target Mac")
        XCTAssertEqual(simplifiedChinese.text(.setupTargetRootPlaceholder), "填写目标 Mac 上的目录路径")
