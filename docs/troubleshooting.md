...
| Session receipt status is not `published` | Interrupted or incomplete transaction state. | `.supermover/sessions/<session>/`, receipt/session files, command logs. | Preserve evidence. Use planned `recover` once available; for now, triage manually before rerun. |
