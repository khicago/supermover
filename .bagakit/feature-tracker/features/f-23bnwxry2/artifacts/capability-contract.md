...
| --- | --- | --- | --- | --- | --- | --- |
| Profile | Create profile | `profile init --profile --source --target [--target-id] [--id] [--name]` | Missing | Native source/target path pickers write profile through CLI; app or new CLI preflight must verify readable source and safe/writable target before green setup state | Missing source path, missing target path, existing profile path, unreadable source, unsafe target, missing profile id/name decision | CLI smoke with temp roots; Swift model test for disabled create |
